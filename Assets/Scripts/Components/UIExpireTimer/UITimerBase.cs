...
namespace Chanto
{
    ...
    public abstract class UITimerBase : MonoBehaviour
    {
        ...
        protected const int UNIT_SECOND = 1;
        protected const int UNIT_MINUTE = 1 << 1;
        protected const int UNIT_HOUR = 1 << 2;
        protected const int UNIT_DAY = 1 << 3;
        protected const int UNIT_ALL = UNIT_SECOND | UNIT_MINUTE | UNIT_HOUR | UNIT_DAY;
        protected readonly static int[] UNIT_ARRAY = new int[] { UNIT_SECOND, UNIT_MINUTE, UNIT_HOUR, UNIT_DAY };
        protected readonly static long[] UNIT_TOTAL_TICKS = new long[] { 10000000, 600000000, 36000000000, 864000000000 };
        protected const int UNIT_COUNT = 4;
        ...
        // 显示天时是否本地化
        public bool useLocalDay = false;

        // 倒计时结束时发送的事件
        public EventId finishEventId = EventId.None;
        ...
        public virtual DateTime GetCurrentTime()
        {
            //if (useServerTime)
            //{
            //    return new DateTime(GameEntry.Timer.GetServerTime() * 10000);
            //}
        ...
        {
            if (finishEventId != EventId.None)
                GameEntry.Event.Fire(finishEventId);
        }
    }
}