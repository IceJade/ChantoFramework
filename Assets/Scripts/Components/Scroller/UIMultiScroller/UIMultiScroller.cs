...
//       scroller.OnDragEndEvent = OnDragEnd;      // 触摸滑动结束回调
//    3.实现创建Item的回调
...
public class UIMultiScroller : MonoBehaviour, IEndDragHandler
{
    ...
    public delegate void OnEndDragHandler(PointerEventData eventData);

    public OnEndDragHandler OnDragEndEvent = null;
    ...
    public void reloadDataForCurrentPos()
    {
        ForceUpdateItems();
    }
    ...
    private void CreateItem(int index)
    {
        ...
        else
        {
            ...
            //GameObject obj = ObjectExtension.CreateInstClone(itemPrefab);
    ...
    private void GetBestPerLine()
    {
        ...
        switch (_movement)
        {
            case Arrangement.Horizontal:
                {
                    this.maxPerLine = Math.Max(1, (int)((this._content.rect.height + this.spacingY - this._Padding.top) / (this.cellHeight + this.spacingY)));

                    break;
                }
            case Arrangement.Vertical:
                {
                    this.maxPerLine = Math.Max(1, (int)((this._content.rect.width + this.spacingX - this._Padding.left) / (this.cellWidth + this.spacingX)));
