...
[CustomEditor(typeof(IMTextMeshProUGUI), true), CanEditMultipleObjects]
public class IMTextMeshProUGUIEditor : TMP_EditorPanelUI
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if(_isFontSetting)
        {
            ...
            {
                var tmp = (IMTextMeshProUGUI)target;
                if (null != tmp)
        ...
        rect = EditorGUILayout.GetControlRect(false, 24);
        if (GUI.Button(rect, "同步字体大小"))
        {
            var tmp = (IMTextMeshProUGUI)target;
            if (null != tmp)
                tmp.setSystemFontSize(tmp.fontSize);
        }
