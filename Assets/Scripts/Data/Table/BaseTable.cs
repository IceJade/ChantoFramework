...
namespace Chanto
{
	...
	public class BaseTable
    {
        ...
        protected virtual void LoadTable()
        {
            ...
            if (this.IsLoading())
            {
                Log.Warning($"the table {this.GetTableFileName()} is loading, please wait...");
                return;
            }
            ...
            {
                Log.ErrorFormat("The file {0} is not load, please check tables assetbundle", this.GetTableFileName());
            }
#endif
#else
            ...
            using (UnityWebRequest request = UnityWebRequest.Get(filepath))
            {
                ...
                while (!request.downloadHandler.isDone)
                {
                    if (request.isNetworkError || request.isHttpError)
                    {
                        ...
                        Log.ErrorFormat("Load {0} fail, error mssage : {1}", request.url, request.error);
                    ...
                    {
                        this.load_state = E_LoadState.Fail;
                        Log.ErrorFormat("Load table {0} time out.", filepath);
            ...
            {
                this.load_state = E_LoadState.Fail;
                Log.ErrorFormat("Read table {0} exception, error msg : {1}", filepath, e.Message);
            }
#endif
            ...
            {
                this.load_state = E_LoadState.Fail;
                Log.ErrorFormat("Load table {0} fail.", filepath);
            }
        ...
        protected virtual void LoadIndex()
        {
            ...
            {
                Log.ErrorFormat("The file {0} is not load, please check tables assetbundle", this.GetIndexFileName());
            }
#endif
#else
...
//                 {
//                     // 如果读写区存在表文件, 那么使用此文件
//                     string abFullPathFile = Path.Combine(Application.persistentDataPath, "Tables", abFileName);
...
//             {
//                 // 如果读写区存在表文件, 那么使用此文件
//                 string fullPathFile = Path.Combine(Application.persistentDataPath, "Tables", dataFileName);
...
//             }
...
#if UNITY_EDITOR
            return Path.Combine(Application.dataPath, "Main/DataTable/Tables", dataFileName);
#else
        ...
        protected virtual string GetIndexFile()
        {
            ...
            if (IsUseAbTest ())
            {
                ...
                {
                    // 如果读写区存在表文件, 那么使用此文件
                    string abFullPathFile = Path.Combine(Application.persistentDataPath, "Tables/ids", abFileName);
                    if (File.Exists(abFullPathFile))
                    {
#if !UNITY_EDITOR && !UNITY_IOS
                        return "file://" + abFullPathFile;  
                    ...
                    {
                        Log.Error($"临时目录没有找到配表的索引文件:{abFullPathFile}");
                    }
                }
...
//             {
//                 // 如果读写区存在表文件, 那么使用此文件
//                 string fullPathFile = Path.Combine(Application.persistentDataPath, "Tables/ids", indexFileName + ".bytes");
//                 if (File.Exists(fullPathFile))
//                 {
// #if !UNITY_EDITOR && !UNITY_IOS
//                     return "file://" + fullPathFile;
