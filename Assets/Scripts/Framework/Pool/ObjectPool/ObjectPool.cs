...
namespace Framework.Pool
{
    ...
    public class ObjectPool<T> : ObjectPoolBase where T : ObjectBase
    {
        ...
        public void Unspawn(object target)
        {
            ...
            throw new FrameworkException(string.Format("Can not find target in object pool '{0}'.", Utility.Text.GetFullName<T>(Name)));
        }
        ...
        public override void Release(int toReleaseCount)
        {
            ...
            if (canReleaseObjects.Count <= 0)
            {
                return;
            }
            LinkedList<T> toReleaseObjects = GetReleaseObjects(canReleaseObjects, toReleaseCount, expireTime);
            ...
            foreach (ObjectBase toReleaseObject in toReleaseObjects)
            {
                ...
                {
                    throw new FrameworkException("Can not release null object.");
                }
                ...
                if (!found)
                {
                    throw new FrameworkException("Can not release object which is not found.");
                }
            }
        }
        ...
        private LinkedList<T> GetReleaseObjects(LinkedList<T> candidateObjects, int toReleaseCount, DateTime expireTime)
        {
