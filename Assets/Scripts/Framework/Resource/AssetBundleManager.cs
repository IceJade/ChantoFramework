...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        {
            BaseLocalURL = string.IsNullOrEmpty(path) ? Utility.GetStreamingAssetsDirectory() : path;
...
//#endif
        ...
        void Update()
        {
            ...
            if (m_InProgressOperations.Count > 0)
            {
                ...
                while (i < m_InProgressOperations.Count)
                {
                    ...
                    {
                        //if (Log.IsLoad())
                            Log.Info("Handled " + (count - m_InProgressOperations.Count));
                        break;
                    }
                }
            }
        ...
        private static object LoadSimulateAssetSync(string assetBundleName, string assetName, System.Type type)
        {
            ...
            if (!string.IsNullOrEmpty(assetName))
            {
                ...
                {
                    Log.Warning(string.Format("There is no asset with name \"{0}\" in \"{1}\" with type {2}", assetName, variant, type));
                }
            ...
            else
            {
                ...
                {
                    Log.Warning("There is no assetbundle with name " + variant);
                }
            }
#endif
            return null;
        ...
        private static LoadedAssetBundle LoadBundleSync(string assetBundleName)
        {
            ...
            if (bundle == null)
            {
                Log.Warning($"{assetBundleName} is not exist.");
            }
            else
            {
#if WRITE_BUNDLE_LOG
...
#if UNITY_EDITOR
            if (Log.IsLoad())
                Log.InfoFormat(LOADSTR, assetName, type, assetBundleName, Time.realtimeSinceStartup);
...
#if UNITY_EDITOR
            if (Log.IsLoad())
                Log.InfoFormat(LOADSTR, levelName, typeof(UnityEngine.SceneManagement.Scene), assetBundleName, Time.realtimeSinceStartup);
