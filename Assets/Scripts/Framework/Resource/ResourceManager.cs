...
namespace Framework
{
    ...
    public class ResourceManager : SingletonBehaviour<ResourceManager>
    {
        ...
        public static string AssetKeyLower(string assetBundle, string assetName, System.Type type)
        {
            ...
            {
                Log.Error("AssetBundleName is Null");
                return string.Empty;
            }
        ...
        public static void ParseAssetPath(string assetPath, bool isSingleBundle, out string assetName,
            out string assetBundle)
        {
        ...
        /// <summary>
        /// 使用方法, GameObject 添加自动管理的组件
        /// </summary>
        /// <param name="obj"></param>
        /// <param name="key"></param>
        /// <returns></returns>
        private object MakePrefabPool(object obj, string key)
        {
            if (!CloseAutoPool && obj is GameObject prefab)
            {
                //引用计数挂在池子上
                if (UsedObjectPool.HavePool(prefab))
                {
                    AutoUnloadAssetWithKey(key);
                    return obj;
                }
        ...
        public T LoadAssetSync<T>(string assetPath) where T : Object
        {
            ...
            // 如果缓存区已经有资源直接回调
            if (cache.obj != null)
                return MakePrefabPool(cache.obj, key) as T;
            ...
            OnLoadComplete callback = null, MemeryHold memeryHold = MemeryHold.Normal)
        {
            ...
            if (cache.obj != null)
            {
                callback?.Invoke(key, MakePrefabPool(cache.obj, key), null);
                return key;
            }
        ...
        public void UnloadAssetWithKey(string key, bool immediately = false, bool autoUnload = false)
        {
            ...
            if (m_AssetCaches.TryGetValue(key, out AssetCache cache))
            {
                // 池子的 prefab 交给池子管理, 主动调用释放无效
                if (autoUnload == false && cache.obj is GameObject go && UsedObjectPool.HavePool(go))
                {
                    Log.Error($"UnloadAssetWithKey:{key} is prefab in UsedObjectPool");
                    return;
                }
                    ...
                    if (m_AssetCaches.TryGetValue(key, out AssetCache cache))
                    {
                        ...
                        else
                        {
                            if (string.IsNullOrEmpty(operation.Error) == false)
                                Log.WarningFormat("Load Assets {0} has error {1}", operation.AssetName,
                                    operation.Error);
                            // 读取所有Assets或者指定Asset
        ...
        private AssetCache CacheAsset(string key, object obj)
        {
            ...
            {
                Log.WarningFormat("There is no asset cache with key : {0}, so {1} can not be cached.", key,
                    ((Object)obj).name);
            }
        ...
        private AssetCache GenAssetCache(string key, string assetBundle, string assetName, System.Type type,
            MemeryHold hold)
        {
        ...
        // /// </summary>
        public void LoadGameObjectAsync(string assetPath, System.Action<GameObject> callback,
            bool isSingleBundle = true)
        {
