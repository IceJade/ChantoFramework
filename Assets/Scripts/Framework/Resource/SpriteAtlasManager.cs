...
namespace Framework
{
    public class SpriteAtlasManager : SingletonBehaviour<SpriteAtlasManager>
    {
        ...
        public void PreloadSpriteAtlas(string atlasName, MemeryHold memeryHold = MemeryHold.Normal, System.Action<SpriteAtlas> action = null)
        {
            ...
            {
                //if (Log.IsLoad())
                Log.InfoFormat("+++! PreloadSpriteAtlas LoadAssetAsync ok {0}, err {1}", atlasName, err);
        ...
        {
            var datarow = GameEntry.Table.GetDataRow<AtlasDataRow>(atlasName);
            if (null != datarow)
        ...
        private void LoadSpriteAtlas(string spriteName, System.Action<SpriteAtlas> action)
        {
            ...
            var spriteDataRow = GameEntry.Table.GetDataRow<SpritesDataRow>(spriteName);
            if (null != spriteDataRow)
            {
                if (spriteDataRow.LoadType == 0)
                {
                    Log.Error($"{spriteName}应该为AB加载模式而非图集加载模式, 请检查!");
                    action?.Invoke(null);
                ...
                {
                    Log.Error($"{spriteName}在sprites配置中的图集名称为空, 请检查!");
                    action?.Invoke(null);
                    return;
                }

                var atlasDataRow = GameEntry.Table.GetDataRow<AtlasDataRow>(atlasName);
                if (null != atlasDataRow)
                    assetBundle = atlasDataRow.BundleName;
            }
            else
            {
                Log.Error($"sprites配置文件中没有查到{spriteName}, 请检查!");
                action?.Invoke(null);
                return;
            }
        ...
        public void GetSpriteAsync(string spriteName, System.Action<Sprite> callback)
        {
            ...
            {
                Log.Error("spriteName is null, in GetSpriteAsync function");
            ...
            LoadSpriteAtlas(spriteName, (spriteAtlas) =>
             {
                 ...
                 if (spriteAtlas == null)
                 {
                     Log.Error($"{spriteName}没有查到所在图集, 请检查!");
                     callback?.Invoke(DefaultSprite);
                     return;
                 }
                 ...
                 Log.Error("sprite is null, in GetSpriteAsync function");
                 callback?.Invoke(DefaultSprite);
             });
