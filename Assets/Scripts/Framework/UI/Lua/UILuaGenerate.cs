...
public class LuaCodeOverwrite
{
    ...
    public void generate(UILuaForm com, string path)
    {
    ...
    {
        foreach (var item in thisObjList)
        {
            if (item == objName)
            {
                return true;
                ...
                if (null != component)
                {
                    ...
                    else if (type == LuaComType.Toggle || type == LuaComType.Slider)
                    {
                        ...
                        if (checkEventFunc(finder.Trim(), ss) == false)
                        {
                            ...
                            format += "\t\tonValueChanged_${ToggleName}\n";
                            format += "\tend)";
                            string line = format.Replace("${ToggleName}", exportName);
                            addEventList += line;

                            string format3 = "\t${ToggleName}.onValueChanged:RemoveAllListeners()";
    ...
    public void parserSpecialFunc()
    {
        int beginLine = -1;
        int endLine = -1;
        if (findFunc("InitControl", out beginLine, out endLine))
        {
...
local ${ClassName} = class($${ClassName}, UIBase)
local M = ${ClassName}
...
--初始化--
function M:OnInit(transform, userData)
    -- basic
    self:InitControl()
end
