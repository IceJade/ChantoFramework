...
public class UILuaForm : UIBaseView
{
    ...
    [BlackList]
    [Header("Lua组件分组")]
    public LuaComGroup[] LuaComGroups;
    ...
    protected internal override void OnInit(object userData)
    {
        ...
        if (null == scriptEnv)
        {
            ...
            {
                Log.Error($"lua file {Constant.Lua.UILuaBridgeFile} is error, please check lua config...");
                return;
            }
    ...
    private void outputLuaFile()
    {
        ...
        else
        {
            ...
            if (lastOpenPath.IsNotNullAndEmpty())
                luaDir = lastOpenPath;
        }
    ...
    private void OnSelectedTarget()
    {
        ...
        for (int i = 0; i < this.LuaComGroups.Length; i++)
        {
            for (int j = 0; j < this.LuaComGroups[i].LuaComs.Length; j++)
            {
                ...
                {
                    isFinded = true;
                    Debug.LogErrorFormat("Group:" + this.LuaComGroups[i].Name + ", Name:" + this.LuaComGroups[i].LuaComs[j].Name + ", Type:" + this.LuaComGroups[i].LuaComs[j].Type.ToString());
                }
            }
        }
